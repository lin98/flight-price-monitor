...
最小可用的機票價格監測工具：列出 **2027-03-01 ～ 2027-05-31** 所有 5 天行程
（出發日 + 4 晚，回程日含在內），計算每段行程要請幾天假，並嘗試查詢真實票價。
...
151 個測試，全部離線（瀏覽器層在測試裡被替身取代，解析層吃 `tests/fixtures/` 的頁面快照）。
...
網頁提供指定日期查價、固定停留晚數的便宜日期排行，以及同一航班的每日／逐次報價走勢。每次實際查詢自動存入既有 SQLite 歷史資料庫；每日排程尚未啟用。價格為一位成人經濟艙直飛，來回組合是兩張單程相加。服務僅限這台電腦存取。
