...
在自己電腦上跑的直飛機票查價工具：讀 Google Flights 當下顯示的價格，資料全部留在本機。

- **連假便宜機票**（網頁首頁）：依行政院人事行政總處的辦公日曆表找出最近的連假，
  排出「不請假／前後各請一天」的走法，比較 8 個熱門直飛地點哪裡、哪幾天最便宜
- **任意航線查價**：指定日期列出所有直飛班次與組合，或掃描一段期間找便宜日期
- **同一航班的票價歷史**：每次實際查價都寫進 SQLite，看得到某一班每天的漲跌
- **定點監測**（最早的功能）：TPE/KHH→PUS 2027/3–5 月每天的 5 天 4 夜行程、請假天數與多人會合比價
...
按排行上的「看全部航班」，會帶著日期跳到指定日期查價，列出那兩天所有直飛班次的組合：
...
```bash
python -m fare_watch.holiday_deals TPE 2026-09-25 2026-09-28   # 連假第一天、最後一天
```
